...
def add_layers(input_layer, description):
    x = input_layer
    layers = re.split(r",\s*(?![^\[\]]*])", description)
    for layer_descr in layers:
        x = add_layer(x, layer_descr)
    return x


def add_layer(input_layer, description):
    types = {
        "C": add_convolution,
    ...
    return types[layer_type](input_layer, param_string)


def add_convolution(input_layer, param_string):
    params = param_string.split("-")
    filters, kernel_size, stride, padding = *map(int, params[:3]), params[3]
    return tf.keras.layers.Conv2D(filters, kernel_size, stride, padding, activation="relu")(input_layer)


def add_convolution_with_bn(input_layer, param_string):
    params = param_string.split("-")
    filters, kernel_size, stride, padding = *map(int, params[:3]), params[3]
    x = tf.keras.layers.Conv2D(filters, kernel_size, stride, padding, use_bias=False)(input_layer)
    x = tf.keras.layers.BatchNormalization()(x)
    x = tf.keras.layers.ReLU()(x)
    return x


def add_max_pool(input_layer, param_string):
    params = param_string.split("-")
    pool_size, stride = map(int, params)
    return tf.keras.layers.MaxPool2D(pool_size, stride)(input_layer)


def add_residual(input_layer, param_string):
    layers_descr = param_string[1:-1]
    output = add_layers(input_layer, layers_descr)
    return tf.keras.layers.Add()([input_layer, output])


def add_flatten(input_layer, _):
    return tf.keras.layers.Flatten()(input_layer)


def add_hidden(input_layer, param_string):
    size = int(param_string)
    return tf.keras.layers.Dense(size, activation="relu")(input_layer)


def add_dropout(input_layer, param_string):
