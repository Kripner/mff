...
class Model(tf.keras.Model):
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        common_input = tf.keras.layers.Input(shape=[MNIST.H, MNIST.W, MNIST.C])
        common_x = tf.keras.layers.Conv2D(10, 3, 2, "valid", activation=tf.nn.relu)(common_input)
        common_x = tf.keras.layers.Conv2D(20, 3, 2, "valid", activation=tf.nn.relu)(common_x)
        common_x = tf.keras.layers.Flatten()(common_x)
        common_x = tf.keras.layers.Dense(200, activation=tf.nn.relu)(common_x)
        common_net = tf.keras.Model(common_input, common_x)

        embed_1 = common_net(images[0])
        embed_2 = common_net(images[1])
        ...
        concat = tf.keras.layers.Concatenate()([embed_1, embed_2])
        direct_x = tf.keras.layers.Dense(200, activation=tf.nn.relu)(concat)
        direct_comparison = tf.keras.layers.Dense(1, activation=tf.nn.sigmoid)(direct_x)

        classification_layer = tf.keras.layers.Dense(10, activation=tf.nn.softmax)
        digit_1 = classification_layer(embed_1)
        digit_2 = classification_layer(embed_2)

        indirect_comparison = tf.keras.layers.Lambda(lambda inp: tf.argmax(inp[0], axis=1) > tf.argmax(inp[1], axis=1))([digit_1, digit_2])
        outputs = {
    ...
    def create_dataset(
        ...
        if training:
            dataset = dataset.shuffle(10_000, seed=args.seed)
        ...
        def create_element(images, labels):
            comparison = int(labels[0] > labels[1])
            return (
                (images[0], images[1]),
                {
                    "digit_1": labels[0],
