...
class WithAttention(tf.keras.layers.AbstractRNNCell):
    ...
    def call(self, inputs, states):
        ...
        state = states[0]
        decoded_projected = self._project_decoder_layer(state)
        projected = self._encoded_projected + tf.expand_dims(decoded_projected, axis=1)
        logits = self._output_layer(tf.nn.tanh(projected))
        weights = tf.nn.softmax(logits, axis=1)
        attention = tf.reduce_sum(self._encoded * weights, axis=1)
        with_attention = tf.concat([inputs, attention], axis=1)
        return self._cell(with_attention, states)
...
class Model(tf.keras.Model):
    def __init__(self, args: argparse.Namespace, train: MorphoDataset.Dataset) -> None:
        ...
        self._source_embedding = tf.keras.layers.Embedding(
            input_dim=len(self._source_mapping.get_vocabulary()),
        ...
        self._target_output_layer = tf.keras.layers.Dense(
            units=len(self._target_mapping.get_vocabulary()),
        )
        ...
        if not args.tie_embeddings:
            ...
            self._target_embedding = tf.keras.layers.Embedding(
                input_dim=len(self._target_mapping.get_vocabulary()),
                output_dim=args.cle_dim,
            )
        else:
            ...
            self._target_embedding = lambda target_ids: \
                tf.gather(tf.transpose(self._target_output_layer.kernel), target_ids) * tf.sqrt(float(args.rnn_dim))
    ...
    def encoder(self, inputs: tf.Tensor) -> tf.Tensor:
        ...
        inputs_embed = self._source_embedding(inputs)
        ...
        return self._source_rnn(inputs_embed).to_tensor()
    ...
    def decoder_training(self, encoded: tf.Tensor, targets: tf.Tensor) -> tf.Tensor:
        ...
        tf.debugging.assert_equal(targets[:, -1:], MorphoDataset.EOW)
        decoder_inputs = tf.concat([
            tf.cast(tf.fill([*tf.shape(targets)[:-1], 1], MorphoDataset.BOW), tf.int64),
            targets[:, :-1],
        ], axis=-1)
        ...
        decoder_inputs_embed = self._target_embedding(decoder_inputs)
        decoder_rnn = self._target_rnn(decoder_inputs_embed, initial_state=[encoded[:, 0]])
        logits = self._target_output_layer(decoder_rnn)
        return logits
    ...
    def decoder_inference(self, encoded: tf.Tensor, max_length: tf.Tensor) -> tf.Tensor:
        ...
        inputs = tf.cast(tf.fill([batch_size], MorphoDataset.BOW), dtype=tf.int64)
        states = [encoded[:, 0]]
        ...
        while tf.math.logical_and(index < max_length, tf.math.reduce_any(result_lengths == max_length)):
            ...
            rnn_outputs, states = self._target_rnn.cell(inputs_embed, states)
            logits = self._target_output_layer(rnn_outputs)
    ...
    def train_step(self, data):
        ...
        x_flat = tf.strings.unicode_split(x_flat, input_encoding="UTF-8")
        ...
        y_flat = tf.strings.unicode_split(y_flat, input_encoding="UTF-8")
        y_flat = self._target_mapping(y_flat)
        y_flat = tf.concat(
            [
                y_flat,
                tf.cast(tf.fill([*tf.shape(y_flat)[:-1], 1], MorphoDataset.EOW), tf.int64)
            ],
            axis=-1,
        )
    ...
    def predict_step(self, data):
        ...
        #   tensor with individual characters as strings,
        # - `self._source_mapping` to remap the character strings to ids.
        data_flat = tf.strings.unicode_split(data_flat, input_encoding="UTF-8")
