...
class Model(tf.keras.Model):
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        if args.rnn == "LSTM":
            rnn = tf.keras.layers.LSTM(args.rnn_dim, return_sequences=True)(sequences)
        elif args.rnn == "GRU":
            rnn = tf.keras.layers.GRU(args.rnn_dim, return_sequences=True)(sequences)
        elif args.rnn == "SimpleRNN":
            rnn = tf.keras.layers.SimpleRNN(args.rnn_dim, return_sequences=True)(sequences)
        else:
            raise Exception()
        ...
        if args.hidden_layer > 0:
            hidden = tf.keras.layers.Dense(args.hidden_layer, activation=tf.nn.relu)(rnn)
        else:
            hidden = rnn
