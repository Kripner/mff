...
def main(args: argparse.Namespace) -> Tuple[List[float], List[float]]:
    ...
    for model in range(args.models):
        ...
        ensemble_input = tf.keras.layers.Input(shape=[MNIST.H, MNIST.W, MNIST.C])
        ensemble_output = tf.keras.layers.Average()([models[i](ensemble_input) for i in range(model + 1)])
        ensemble = tf.keras.Model(inputs=ensemble_input, outputs=ensemble_output)
        ensemble.compile(metrics=[tf.metrics.SparseCategoricalAccuracy(name="accuracy")])

        _, ensemble_accuracy = ensemble.evaluate(mnist.dev.data["images"], mnist.dev.data["labels"])
