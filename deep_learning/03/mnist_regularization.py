...
def main(args: argparse.Namespace) -> Dict[str, float]:
    ...
    #     new_labels = np.ones([dataset.size, MNIST.LABELS], dtype=float) * (args.label_smoothing / (MNIST.LABELS - 1))
    #     new_labels[np.arange(dataset.size), dataset.data["labels"]] = 1 - args.label_smoothing
    #     assert(np.allclose(np.sum(new_labels, axis=1), 1))
    #     dataset.data["labels"] = new_labels

    for dataset in [mnist.train, mnist.dev, mnist.test]:
        dataset.data["labels"] = tf.one_hot(dataset.data["labels"], MNIST.LABELS)

    # ! TODO: try "confidence filtering"
