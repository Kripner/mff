...
parser.add_argument("--hidden_layer_size", default=256, type=int, help="Size of a hidden layer.")
parser.add_argument("--hidden_layer_count", default=1, type=int, help="Number of the hidden layers.")
...
def evaluate_model(
        model: tf.keras.Model, seed: int = 42, episodes: int = 100, render: bool = False,
...
def main(args: argparse.Namespace) -> Optional[tf.keras.Model]:
    ...
    if not args.evaluate:
        ...
        model.add(tf.keras.layers.Input(len(observations[0])))
        ...
        steps = args.epochs * (data_size / args.batch_size)
        ...
        class EvalCallback(tf.keras.callbacks.Callback):
            def on_epoch_end(self, epoch, logs=None):
                evaluation = evaluate_model(model, episodes=15)
                print(f"\nEvaluation after epoch {epoch + 1}: {evaluation}")
                logs["evaluation"] = evaluation
        ...
        model.fit(
            observations, labels, batch_size=args.batch_size, epochs=args.epochs, callbacks=[EvalCallback(), tb_callback, stopping_cbk]
        )
