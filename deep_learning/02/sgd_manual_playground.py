...
class Model(tf.Module):
    ...
    def train_epoch(self, dataset: MNIST.Dataset) -> None:
        for batch in dataset.batches(self._args.batch_size):
            ...
            # Test the implementation against audograd
            with tf.GradientTape() as tape:
                probabilities, _, _ = self.predict(tf.convert_to_tensor(batch["images"]))
                predicted_probs = tf.gather(probabilities, tf.convert_to_tensor(batch["labels"], dtype=tf.int32), batch_dims=1)
                loss = -tf.math.reduce_mean(tf.math.log(predicted_probs))
            auto_gradients = tape.gradient(loss, variables)

            close = []
            diffs = []
            for grad, auto_grad in zip(gradients, auto_gradients):
                # if not np.allclose(grad, auto_grad):
                close.append(tf.experimental.numpy.allclose(grad, auto_grad).numpy())
                diffs.append(tf.norm(grad - auto_grad).numpy())
            print(f"close: {close} | max diff.: {diffs}")
