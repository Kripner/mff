...
def main(args: argparse.Namespace) -> np.ndarray:
    ...
    def choose_next_action(Q: np.ndarray, state) -> tuple[int, float, bool]:
        greedy_action = argmax_with_tolerance(Q[state])
        greedy = generator.uniform() >= args.epsilon
        next_action = greedy_action if greedy else env.action_space.sample()
        return next_action, args.epsilon / env.action_space.n + (1 - args.epsilon) * (
                    greedy_action == next_action), greedy
    ...
    def sarsa():
        ...
        next_action, next_action_prob, _ = choose_next_action(Q, next_state)
        buffer = collections.deque()
        g = 0
        gamma_n = pow(args.gamma, args.n)
        rho = 1
        if args.off_policy:
            rhos = []
        while not done:
            policy = compute_target_policy(Q)
            ...
            if not done:
                next_action, next_action_prob, _ = choose_next_action(Q, next_state)
                if args.off_policy:
                    rhos.append(policy[next_state, next_action] / next_action_prob)

            buffer.appendleft((state, action, action_prob, reward))
            g += reward * (gamma_n / args.gamma)
            if len(buffer) == args.n:
                past_state, past_action, past_action_prob, past_reward = buffer.pop()
                val_estimate = g
                if not done:
                    target_policy = compute_target_policy(Q)
                    if args.mode == "sarsa":
                        curr_val_estimate = Q[next_state, next_action]
                        if args.off_policy:
                            rho = np.prod(rhos)
                    elif args.mode == "expected_sarsa":
                        curr_val_estimate = np.dot(target_policy[next_state, :], Q[next_state, :])
                        if args.off_policy:
                            rho = np.prod(rhos[:-1])
                    else:
                        raise Exception()
                    val_estimate += gamma_n * curr_val_estimate
                Q[past_state, past_action] += args.alpha * rho * (val_estimate - Q[past_state, past_action])

                g = (g - past_reward) / args.gamma

                if args.off_policy and len(rhos) > 0:
                    del rhos[0]
            else:
                g = g / args.gamma
    ...
    def tree_backup():
        ...
        while True:
            if T is None:
                action, _, greedy = choose_next_action(Q, state)
                next_state, reward, terminated, truncated, _ = env.step(action)
                done = terminated or truncated
                if done:
                    T = t + 1
            ...
            if tau >= 0:
                ...
                if T is not None and t + 1 >= T:
                    ...
                    G = reward + args.gamma * np.dot(policy[next_state], Q[next_state])
                for s_k, a_k, greedy_k, r_k in buffer:
                    avg_return = 0
                    for a in range(env.action_space.n):
                        avg_return += policy[s_k, a] * (G if a == a_k else Q[s_k, a_k])
                    G = r_k + args.gamma * avg_return
                if len(buffer) > 0:
                    s_tau, a_tau, _, _ = buffer.pop()
                else:
                    s_tau, a_tau = state, action
                Q[s_tau, a_tau] += args.alpha * (G - Q[s_tau, a_tau])

            if T is None:
                if args.n != 1:
                    buffer.appendleft((state, action, greedy, reward))

                state = next_state

            if T is not None and tau == T - 1:
                break
            t += 1
