...
class MultiArmedBandits():
    ...
    def random_action(self) -> int:
        return self.__generator.randint(len(self.__bandits))
...
#            action = random.randint(0, args.bandits - 1)
            action = random_generator.randint(args.bandits)
#            action = env.random_action()
        ...
        counts[action] += 1
        rate = 1 / counts[action] if args.alpha == 0 else args.alpha
        estimates[action] += rate * (reward - estimates[action])
