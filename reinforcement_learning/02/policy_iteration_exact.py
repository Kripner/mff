...
def main(args: argparse.Namespace) -> tuple[list[float], list[int]]:
    ...
    actions_values = np.zeros([len(GridWorld.actions)], dtype=float)
    for _ in range(args.steps):
        # Evaluation
        eval_matrix = np.zeros([GridWorld.states, GridWorld.states], dtype=float)
        eval_rightside = np.zeros([GridWorld.states], dtype=float)
        for s in range(GridWorld.states):
            a = policy[s]
            for prob, reward, new_s in GridWorld.step(s, a):
                eval_rightside[s] -= prob * reward
                eval_matrix[s, new_s] += prob * args.gamma
            eval_matrix[s, s] -= 1
        value_function = np.linalg.solve(eval_matrix, eval_rightside)
        ...
        for s in range(GridWorld.states):
            for a in range(len(GridWorld.actions)):
                v = 0
                for prob, reward, new_s in GridWorld.step(s, a):
                    v += prob * (reward + args.gamma * value_function[new_s])
                actions_values[a] = v
            policy[s] = argmax_with_tolerance(actions_values)
