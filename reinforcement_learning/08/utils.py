...
def add_universal_arguments(parser: argparse.ArgumentParser, default_directory=None):
    ...
    parser.add_argument("--evaluate_each", type=int, default=200, help="Period of evaluation, used when training.")
    parser.add_argument("--evaluate_for", type=int, default=64, help="How many episodes to evaluate.")
...
class TemplateAlgorithm(ABC):
    def __init__(self):
        ...
        self.config = None
        self.logger = None
        self.training = None  # Whether training is currently in progress.
    ...
    @classmethod
    @abstractmethod
    def load_from(cls, filename, simple_env=None):
        pass
    ...
    @classmethod
    def load_from_smart(cls, filename, simple_env, args):
        if filename.endswith(".h5"):
            return cls.load_from_eval_only(filename, simple_env, args)
        else:
            return cls.load_from(filename, simple_env)
...
class MainTemplate(ABC):
    ...
    @classmethod
    @abstractmethod
    def init_new_model(cls, simple_env, args):
        pass
    ...
    @classmethod
    def _get_model(cls, simple_env, args):
        ...
        if args.load:
            ...
            return cls.get_algorithm_type().load_from(args.model, simple_env)
        else:
            print("Initializing new model.")
            return cls.init_new_model(simple_env, args)
    ...
    def run(cls, simple_env, args: argparse.Namespace):
        if args.recodex:
            ...
            algorithm = cls.get_algorithm_type().load_from_smart(args.model, simple_env, args)
            ...
            cls._train(simple_env, vector_env, args)
        elif args.action == "evaluate":
            vector_env = cls.get_vector_env(args)
            cls._evaluate(simple_env, vector_env, args)
            ...
            alg = cls.get_algorithm_type().load_from(args.model, simple_env)
    ...
    @classmethod
    def _evaluate(cls, simple_env, vector_env, args, reporting_interval=20):
        if not args.load:
            print("Assuming --load.")
            args.load = True

        algorithm = cls._get_model(simple_env, args)
    ...
    @classmethod
    def _train(cls, simple_env, vector_env, args):
        ...
        algorithm.init_training(vector_env, config, logger, evaluations_num=args.evaluate_for)
    ...
    @classmethod
    def _write_experiment_info(cls, args, blacklist=("./walker.torch.py", "./wrappers.py")):
