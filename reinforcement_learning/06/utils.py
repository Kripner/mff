...
def add_universal_arguments(parser: argparse.ArgumentParser, default_directory=None):
    ...
    parser.add_argument("--evaluate_for", type=int, default=16, help="How many episodes to evaluate.")
...
class Visualizations:
    @staticmethod
    def show_filters(model, layer_idx, max_filters=None):
        layer = model.layers[layer_idx]
        assert "conv" in layer.name
        # get filter weights
    ...
    @staticmethod
    def show_feature_map(model, inp, layer_idx, plot_width=8):
        # redefine model to output right after the first hidden layer
        model = models.Model(inputs=model.inputs, outputs=model.layers[layer_idx].output)
        # expand dimensions so that it represents a single 'sample'
        inp = np.expand_dims(inp, axis=0)
        # get feature map for first hidden layer
        feature_maps = model.predict(inp)
        # plot all 64 maps in an 8x8 squares
        ix = 1
        n_filters = feature_maps.shape[3]
        plot_height = n_filters // plot_width
        for _ in range(plot_width):
            for _ in range(plot_height):
                # specify subplot and turn of axis
                ax = plt.subplot(plot_width, plot_height, ix)
                ax.set_xticks([])
                ax.set_yticks([])
                # plot filter channel in grayscale
